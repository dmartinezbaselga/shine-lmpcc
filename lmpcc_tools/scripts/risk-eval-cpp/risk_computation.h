...
public:
  RiskComputation()
  {
    ...
    ros_markers_.reset(new RosTools::ROSMarkerPublisher(nh_, "/risk/samples", "map", 1800));
...
public:
  void Visualize(const std::vector<Eigen::VectorXd>& x, const std::vector<Eigen::VectorXd>& y)
  {
    ...
    RosTools::ROSPointMarker& sample = ros_markers_->getNewPointMarker("CUBE");
