...
def metric_collisions(experiment_data, metrics):
    ...
    metrics["passive collisions"] = []
    metrics["active collisions"] = []
    metrics["max intrusion"] = []
    for experiment in experiment_data:
        metrics["collisions"].append(int(np.max(experiment["metric_collisions"]) > 0))
        metrics["severe collisions"].append(int(np.max(experiment["max_intrusion"]) > 0.)) # used to be 0.05

        # metrics["max intrusion"] =

        metrics["passive collisions"].append(0)
        collision_times = np.atleast_1d(np.squeeze(np.where(experiment["max_intrusion"] > 0.))[0])
        if len(collision_times) > 0:
            is_passive_collision = True
            for collision in collision_times:
                # Find out what the robot speed is when we collide with an obstacle
                velocity = dist(experiment["vehicle_pose"][collision], experiment["vehicle_pose"][collision-1]) / 0.05

                # If it is always smaller than a low threshold -> it is a passive collision
                if velocity > 0.2: # estimate velocity
                    is_passive_collision = False
                    break

            metrics["passive collisions"][-1] = is_passive_collision

    metrics["num collisions"] = int(np.sum(metrics["collisions"]))
    metrics["num severe collisions"] = int(np.sum(metrics["severe collisions"]))
    metrics["num passive collisions"] = int(np.sum(metrics["passive collisions"]))
    metrics["num active collisions"] = metrics["num severe collisions"] - metrics["num passive collisions"]
...
def metric_lmpcc_usage(experiment_data, metrics):
    if metrics["name"] != "GMPCC":
        metrics["lmpcc used"] = -1
        return

    number_of_guidance_trajectories = metrics["num paths"]["max"] # this is the number of guidance trajectories (i.e., # planners - 1)
    best_planner = merge_experiment_data(experiment_data, "best_planner_idx")

    lmpcc_used = [e for e in best_planner if e == number_of_guidance_trajectories] # the original planner comes after the guidance trajectories
    metrics["lmpcc used"] = len(lmpcc_used) / float(len(best_planner)) * 100.
...
def main(simulation, planner_name, project=PROJECT_FOLDER):
    ...
    experiment_data = split_ros_data(data, filter_faulty=settings.filter_faulty, num_experiments=MAX_EXPERIMENTS)
    ...
    if metrics['num experiments'] == 0:
        ...
        metric_lmpcc_usage(experiment_data, metrics)
