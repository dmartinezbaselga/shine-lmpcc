...
void SafetyCertifier::init(predictive_configuration *config)
{
    config_ = config;

    // Load settings
    risk_ = config->risk_;
    confidence_ = config->confidence_;

    LMPCC_WARN_ALWAYS("Initializing Safety Certifier");

    if (config_->use_trajectory_sampling_)
    {
        max_support_ = config_->n_bar_ + config_->removal_count_;
    }
    else
    {
    ...
    if (config->automatically_compute_sample_size_)
    {
        config->sample_size_ = FindSampleSize(risk_, confidence_, max_support_); // Overwrites the sample size
    }
    else
    {
        ROS_INFO_STREAM("Safety Certifier: Using static sample size (S = " << config_->sample_size_ << ")");
    }

    sample_size_ = config->sample_size_; // save in this class

    // Compute the risk for all allowed support values
    ComputeRiskForSupportRange();
    LMPCC_WARN_ALWAYS("Safety Certifier Initialized");
}
...
int SafetyCertifier::FindSampleSize(double risk, double confidence, int max_support)
{
    ...
    ROS_INFO_STREAM("Safety Certifier:\n\tRisk: " << risk << "\n\tConfidence: " << confidence_ << "\n\tMaximum Support (Removed): " << max_support << " (" << config_->removal_count_ << ")"
...
{
    return 1.0 - std::pow(confidence_ / (double)max_support_, 1.0 / ((double)(sample_size - support))) * (1.0 / rootedNChooseK(sample_size, support, sample_size - support));
}
...
{
    risk_lut_.resize(max_support_);

    for (int k = 0; k < max_support_; k++)
        risk_lut_[k] = 1.0 -
                       std::pow(confidence_ / ((double)max_support_), 1.0 / ((double)(sample_size_ - k))) *
...
{
    return GetRiskForSupport(support_subsample.support_subsample_size_);
}