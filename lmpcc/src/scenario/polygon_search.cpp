...
VerticalEvaluation &RecursionMemory::GetStorage(int size)
{
    ...
    // std::cout << "Requesting storage for " << size << " stored at level " << storage_level << ", usage index: " << usage_indices_[storage_level] << std::endl;
    ...
    if (usage_indices_[storage_level] - 1 > (int)data_[storage_level].size() - 1)
    {
        ...
        // ROS_WARN_STREAM("Polygon Search: Exceeded allocated size at storage level " << storage_level << " of (" << data_[storage_level].size() << ")"
        ...
        // return data_[storage_level - 1][usage_indices_[storage_level - 1] - 1]; // We return storage in the previous level
    }
...
PolygonSearch::PolygonSearch(std::vector<ScenarioConstraint> *halfspaces, Eigen::ArrayXd *y_left, Eigen::ArrayXd *y_right, int n_halfspaces, double range)
...
void PolygonSearch::AddRangeConstraints(const Eigen::Vector2d &pose, const double orientation, Eigen::ArrayXd &a1, Eigen::ArrayXd &a2, Eigen::ArrayXd &b)
{
    ...
    LinearConstraint2D new_constraint;
    for (u_int i = 0; i < 4; i++)
    {
...
void PolygonSearch::SearchSide(const Eigen::Vector2d &pose, const ConstraintSide &side)
{
    ...
    std::vector<Eigen::Vector2d> &result_intersects = side == ConstraintSide::BOTTOM ? result_intersects_bot_ : result_intersects_top_;
    ...
    for (size_t i = 0; i < halfspaces_->size(); i++)
    {
        if ((*halfspaces_)[i].side_ == side                                                                           /* Only use constraints that are on the considered side */
            && !((*halfspaces_)[i].type_ == ObstacleType::DYNAMIC && is_removed_[(*halfspaces_)[i].scenario_->idx_])) /* Do not add removed constraints */
        {
            indices.push_back(i);
        }
    }
    ...
    {
        std::cout << "There is only one relevant constraint, returning that constraint and no intersections!" << std::endl;
    ...
    else
    {
        ...
        RecursiveSearch(values_left.top_idx_,
                        values_right.top_idx_,
                        values_left,
                        intersect_left,
                        intersect_right,
                        0,
...
bool PolygonSearch::Search(const Eigen::Vector2d &pose, double orientation, double x_left, double x_right)
{
    ...
    POLYGON_LOG("Found " << result_indices_bot_.size() << " bottom lines and " << result_intersects_bot_.size() << " intersects");

    // Search for the lines on the top part of the polygon
    POLYGON_LOG("Top search");
    SearchSide(pose, ConstraintSide::TOP);
    POLYGON_LOG("Found " << result_indices_top_.size() << " top lines and " << result_intersects_top_.size() << " intersects");
...
bool PolygonSearch::ConstructPolygon()
{
    ...
    POLYGON_LOG("Left Sweep with " << result_intersects_bot_.size() << " bottom intersects, and " << result_intersects_top_.size() << " top intersects!\n");
    ...
    POLYGON_LOG("Right Sweep with " << result_intersects_bot_.size() << " bottom intersects, and " << result_intersects_top_.size() << " top intersects!\n");
    ...
    //     std::cout << "   Intersect: (" << (*intersects_out_[i])(0) << ", " << (*intersects_out_[i])[1] << ")" << std::endl;
    // }
}
...
bool PolygonSearch::SweepTopAndBottom(bool sweep_from_left, Eigen::Vector2d &intersect_out, int &top_idx, int &bot_idx)
{
    ...
    for (size_t i = 0; i < result_intersects_top_.size() + result_intersects_bot_.size(); i++)
    {
        ...
        if (no_bot_indices || (!no_top_indices && direction * (result_intersects_top_[top_idx](0) - result_intersects_bot_[bot_idx](0)) < 0))
        {
        ...
        // Note that we evaluate just before the intersection (tolerance) to ensure that the order is consistent (at the intersection two lines have the same value)
        ...
        // then the intersection that connects the two is the intersection between the bottom and top lines that we have currently
        // Note that top y is just the highest or lowest value depending on bot or top
        ...
        if (top_values.top_y_ > bot_values.top_y_)
        {
            ...
            findIntersect(top_values.top_idx_,
                          bot_values.top_idx_,
                          intersect_out);
    ...
    // Error handling for when the top and bottom line cannot be connected (happens when the vehicle is not an interior point!)
...
void PolygonSearch::RecursiveSearch(int top_index_left, int top_index_right, const VerticalEvaluation &indices, const Eigen::Vector2d &intersect_left, const Eigen::Vector2d &intersect_right,
                                    int n, const ConstraintSide &side, std::vector<int> &indices_out, std::vector<Eigen::Vector2d> &intersects_out)
{
    ...
    // if (!do_intersect)
    // {
    // ROS_ERROR_STREAM("No intersection found (A: " + std::to_string(halfspaces[left.indices_[0]].constraint_.A_(0, 0)) +
    //                  ", " + std::to_string(halfspaces[left.indices_[0]].constraint_.A_(0, 1)) +
    //                  " | b: " + std::to_string(halfspaces[left.indices_[0]].constraint_.b_(0)) + "), (A: " +
    //                  std::to_string(halfspaces[right.indices_[0]].constraint_.A_(0, 0)) + ", " + std::to_string(halfspaces[right.indices_[0]].constraint_.A_(0, 1)) +
    //                  " | b: " + std::to_string(halfspaces[right.indices_[0]].constraint_.b_(0)));
    // ROS_ERROR_STREAM("Lines are parallel!");
    // }
    ...
    {
        POLYGON_LOG("PolygonSearch: Recursion was terminated early!");
    ...
    VerticalEvaluation &values = EvaluateVerticallyAt(intersect(0), intersect(1), indices, side); // Todo: fix n_top also in the data management
    ...
    if (line_count != 0)
    {
        ...
        RecursiveSearch(top_index_left, values.top_idx_, values, intersect_left, intersect, n + 1, side, indices_out, intersects_out);

        // Search to the right
        RecursiveSearch(values.top_idx_, top_index_right, values, intersect, intersect_right, n + 1, side, indices_out, intersects_out);
    }
    else
    {
        ...
        // Had a nasty error that when indices_out was empty it was 0 while left[0] was also zero.....
        if (!(indices_out.size() > 0 && indices_out.back() == top_index_left))
            indices_out.push_back(top_index_left);
...
VerticalEvaluation &PolygonSearch::EvaluateVerticallyAt(const double x, const std::vector<int> &indices_in, const ConstraintSide &side)
{
    // PROFILE_FUNCTION();
...
VerticalEvaluation &PolygonSearch::EvaluateVerticallyAt(const double x, const double y, const VerticalEvaluation &indices_in, const ConstraintSide &side)
{
    // PROFILE_FUNCTION();
...
VerticalEvaluation &PolygonSearch::FindTopConstraint(const std::vector<int> &indices_in, const Eigen::ArrayXd *y_values, const ConstraintSide &side)
{
...
{
    // std::cout << "intersect: " <<  (*y_left_)(a) << " | " <<   (*y_left_)(b) << " | " << (*y_right_)(a) << " | " << (*y_right_)(b) << std::endl;
    // double r = (*y_right_)(a) - (*y_right_)(b);
...
double PolygonSearch::getArea()
{
    double area = 0.0;
    int n = intersects_out_.size();
    for (int i = 0; i < n - 1; i++)
    {
        area += (*intersects_out_[i])(0) * (*intersects_out_[i + 1])(1);
        area -= (*intersects_out_[i])(1) * (*intersects_out_[i + 1])(0);
    }

    area += (*intersects_out_[n - 1])(0) * (*intersects_out_[0])(1);
    area -= (*intersects_out_[n - 1])(1) * (*intersects_out_[0])(0);
