...
SimpleSimInterface::SimpleSimInterface(ros::NodeHandle &nh, Controller *controller, predictive_configuration *config,
                                       SolverInterface *solver_interface_ptr)
    : Interface(nh, controller, config, solver_interface_ptr)
{
  LMPCC_INFO("Initializing SimpleSim Interface.");
  ...
  state_sub_ = nh.subscribe(config_->robot_state_topic_, 1, &SimpleSimInterface::StateCallBack, this, ros::TransportHints().tcpNoDelay());
  steering_sub_ = nh.subscribe(config_->steering_state_topic_, 1, &SimpleSimInterface::SteeringAngleCallback, this);
  acceleration_sub_ = nh.subscribe(config_->acceleration_state_topic_, 1, &SimpleSimInterface::AccelerationCallback, this);
  tire_forces_Fyf_sub_ = nh.subscribe(tire_forces_Fyf_topic_, 1, &SimpleSimInterface::TireForcesFyfCallback, this);
  tire_forces_Fyr_sub_ = nh.subscribe(tire_forces_Fyr_topic_, 1, &SimpleSimInterface::TireForcesFyrCallback, this);

  // Subscribers for high level data
  obstacle_prediction_sub_ = nh.subscribe(config_->obstacle_prediction_topic_, 1, &SimpleSimInterface::ObstacleCallBack, this);
  reference_path_sub_ = nh.subscribe(config_->waypoint_topic_, 1, &SimpleSimInterface::WaypointsCallback, this);
  ...
  follower_pub_ = nh.advertise<lmpcc_msgs::follower>("lmpcc/follower", 1);
  ...
  // For resetting simple sim
  reset_simple_sim_pub_ = nh.advertise<geometry_msgs::Pose>("bicycle/reset_sim", 1);
  ...
  LMPCC_SUCCESS("SimpleSim Interface Initialized");
}

void SimpleSimInterface::Actuate()
{
  PROFILE_AND_LOG(config_->debug_output_, "Actuate planner directly");
  monitor_->PrintStatus();
  if (config_->use_follower_)
  {
    follower_pub_.publish(SendTrajectoryToFollower(solver_interface_ptr_));
  }
  else
  {
    ...
    // control_msg_.speed = solver_interface_ptr_->control_inputs_.veloity;

    control_msg_.steer = solver_interface_ptr_->control_inputs_.rot_velocity;
...
void SimpleSimInterface::ActuateFollower(double control_throttle, double control_steer)
{
  PROFILE_AND_LOG(config_->debug_output_, "Actuate follower");

  // FOLLOWER_INFO("ActuateFollower");
  ...
  // We use the integrated states as input
  control_msg_.throttle = control_throttle;
  control_msg_.steer = control_steer;
  control_msg_.brake = 0.0;
...
void SimpleSimInterface::ActuateBrake(double deceleration)
{
  monitor_->PrintStatus();
  ...
  // We use the integrated states as input
  if (solver_interface_ptr_->State().v() - deceleration * solver_interface_ptr_->DT < 0.) // Do not drive backwards
    deceleration = solver_interface_ptr_->State().v() / solver_interface_ptr_->DT;
...
void SimpleSimInterface::Reset()
{
  reset_simulation_pub_.publish(std_msgs::Empty()); // Reset simulated pedestrians

  reset_simple_sim_pub_.publish(geometry_msgs::Pose()); // Reset simplesim
...
void SimpleSimInterface::ObstacleCallBack(const lmpcc_msgs::obstacle_array &msg)
{
  PROFILE_AND_LOG(config_->debug_output_, "Obstacle callback");
  // Is also used to read the predictions!
  LMPCC_INFO_STREAM("SimpleSimInterface::ObstacleCallback (with " << msg.obstacles.size() << " obstacles)");
  ...
  LMPCC_INFO_STREAM("SimpleSim Interface: Converted " << data_.dynamic_obstacles_.size() << " obstacles to LMPCC format");
...
void SimpleSimInterface::PreProcessObstacles()
{
  ...
  for (auto &obstacle : obstacle_msg_.obstacles)
  {
    ...
    data_.dynamic_obstacles_.emplace_back(obstacle.id, disc_id, Eigen::Vector2d(obstacle.pose.position.x, obstacle.pose.position.y), config_->r_VRU_);

    data_.dynamic_obstacles_.back().pose_ = obstacle.pose;

    // data_.dynamic_obstacles_.back().PredictConstantVelocity(object.twist, solver_interface_ptr_->DT, solver_interface_ptr_->FORCES_N, 1.0
    // / 9., 1.0);
...
void SimpleSimInterface::StateCallBack(const nav_msgs::Odometry::ConstPtr &msg)
{
  PROFILE_AND_LOG(config_->debug_output_, "State callback");
  ...
  solver_interface_ptr_->State().set_x(
      msg->pose.pose.position.x + 1.577 * cos(solver_interface_ptr_->State().psi())); // for shifting the current coordinates to the center of mass
  ...
  if (controller_->objective_reached_)
  {
    controller_->OnReset();
  }
}
...
void SimpleSimInterface::AccelerationCallback(const geometry_msgs::AccelWithCovarianceStamped &msg)
{
  PROFILE_AND_LOG(config_->debug_output_, "Acceleration callback");
...
void SimpleSimInterface::SteeringAngleCallback(const sensor_msgs::JointState &msg)
{
  PROFILE_AND_LOG(config_->debug_output_, "Steering angle callback");
...
/* Callback for saving the front tire forces*/
void SimpleSimInterface::TireForcesFyfCallback(const std_msgs::Float32 &msg)
{
  PROFILE_AND_LOG(config_->debug_output_, "SimpleSim: Front tire forces callback");
  Fyf_ = msg.data;
}

/* Callback for saving the front tire forces*/
void SimpleSimInterface::TireForcesFyrCallback(const std_msgs::Float32 &msg)
{
  PROFILE_AND_LOG(config_->debug_output_, "SimpleSim: Rear tire forces callback");
  Fyr_ = msg.data;
}

double SimpleSimInterface::getTireForcesFyf() { return Fyf_; }

double SimpleSimInterface::getTireForcesFyr() { return Fyr_; }

void SimpleSimInterface::PlotAllObstacles()
{
...
void SimpleSimInterface::WaypointsCallback(const nav_msgs::Path &msg)
{
  PROFILE_AND_LOG(config_->debug_output_, "Waypoints callback");
...
void SimpleSimInterface::DebugInsertFakeObstacles()
{
  ...
  for (int i = 0; i < config_->max_obstacles_; i++)
  {
    ...
    data_.dynamic_obstacles_.emplace_back(i,                                                                           /** Association ID of the obstacle */
                                          i,                                                                           /** Index of the first disc of this obstacle */
                                          Eigen::Vector2d(fake_pose.position.x, fake_pose.position.y), config_->r_VRU_ /** Obstacle radius */
    );
