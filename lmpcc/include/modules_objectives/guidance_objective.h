/**
 * @file guidance_objective.h
 * @author Oscar de Groot (o.m.degroot@tudelft.nl)
 * @brief Controller Module for computing guidance trajectories in the state-space and following them using MPCC (@see python_forces_code/modules.py)
...
#ifndef __GUIDANCE_OBJECTIVE_H__
#define __GUIDANCE_OBJECTIVE_H__
...
/**
 * @brief Homotopy Guidance controller module extends from the reference path ControlModule to implement MPCC over the trajectory
 * but starting from the robot
 */
class GuidanceObjective : public ReferencePath
{

public:
    GuidanceObjective(ros::NodeHandle &nh, predictive_configuration *config, VehicleRegion *vehicle);
...
public:
    ...
    void ReconfigureCallback(SolverInterface *solver_interface, lmpcc::PredictiveControllerConfig &config,
                             uint32_t level, bool first_callback) override;

    void ExportData(RosTools::DataSaver &data_saver) override;

    void GetMethodName(std::string &name) override; // Overwrites! 

private:
    struct LocalPlanner
    {
        int id;
        std::unique_ptr<RosTools::Spline2D> guidance_path_;
    ...
    std_msgs::Time last_time_received_;
    bool receiving_times_ = false;
    ...
    void LoadGuidanceTrajectory(SolverInterface *solver);
    void InitializeSolverWithGuidance(LocalPlanner &planner);
    void LoadGuidanceTrajectory(LocalPlanner &planner);
...
#endif // __GUIDANCE_OBJECTIVE_H__